...
class DealerUpComingFragment : BaseFragment() {
            ...
            adapter = DealerBookingListAdapter(
                ...
                object : DealerBookingListAdapter.ViewHolderClicks {
                    override fun onClickItem(model: BookingModel, position: Int, status: String) {
                        ...
                        profileDealerViewModel.getUserDetail(model.dealerid)
                    }
                }
            )
        }
    ...
    private fun setObserver() {
        ...
        profileDealerViewModel.userDetailResponse.observe(viewLifecycleOwner) {
            when (it.status) {
                Status.LOADING -> {
                    showProgress(requireContext())
                }
