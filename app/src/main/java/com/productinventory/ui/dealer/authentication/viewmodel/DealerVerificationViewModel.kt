...
@HiltViewModel
class DealerVerificationViewModel @Inject constructor(private val userRepository: UserRepository) :
    ViewModel() {
    var verificationNavigator: ShopperVerificationNavigator? = null
    ...
    var user: DealerUserModel = DealerUserModel()
    ...
    fun checkValidation(): Boolean {
        ...
        if (otpCode.value.isNullOrEmpty() || otpCode.value?.length!! < Constants.INT_6) {
        ...
        docIdRef.get().addOnCompleteListener { task ->
            if (task.isSuccessful) {
                ...
                if (document?.exists() == true) {
                    val userModel = DealerUsersList.getUserDetail(document)
                    document.get(Constants.FIELD_FULL_NAME)?.let {
                        Constants.USER_NAME = it.toString()
                    }
                    document.get(Constants.FIELD_DEALER_ID)?.let {
                        Constants.DEALER_USER_ID = it.toString()
                    }
    ...
    fun addUserData(user: DealerUserModel) {
        ...
        val data = hashMapOf(
            ...
            Constants.FIELD_DEALERR_NAME to user.dealername,
