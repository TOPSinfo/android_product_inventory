    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        try {
            if (model.type == Constants.NOTIFICATION_REQUEST_ADDED_ACCEPTED) {
                ...
                holder.imgNotification.setImageResource(R.drawable.ic_notification_calendar_user)
            } else if (model.type == Constants.NOTIFICATION_REQUEST_ADDED_REJECTED) {
                // booking accepted or created
                holder.imgNotification.setImageResource(R.drawable.ic_notification_calendar_reject)
            } else {
                // reminder
