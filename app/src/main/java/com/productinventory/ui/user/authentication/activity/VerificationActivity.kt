...
import com.productinventory.databinding.ActivityVerificationBinding
...
import com.productinventory.ui.user.authentication.viewmodel.VerificationViewModel
...
class VerificationActivity : BaseActivity(), VerificationNavigator {
    ...
    private fun init() {
        ...
        val phoneNumberWithMasking = phoneNumber!!.replace(
            phoneNumber!!.substring(
                Constants.INT_5, phoneNumber!!.length - 2
            ),
            "******"
        )
    ...
    override fun startPhoneNumberVerification() {
        if (NetworkHelper(this).isNetworkConnected()) {
            ...
            val options = phoneNumber?.let {
                PhoneAuthOptions.newBuilder().setPhoneNumber(it) // Phone number to verify
        ...
        } else binding.root.showSnackBarToast(getString(R.string.please_check_internet))
    }
    ...
    override fun signInWithPhoneAuth() {
        ...
        } else showMessage(getString(R.string.please_check_internet))
    }
        ...
        if (Utility.checkConnection(this@VerificationActivity)) {
            val options = phoneNumber?.let {
                PhoneAuthOptions.newBuilder().setPhoneNumber(it) // Phone number to verify
                    ...
                    .setActivity(this) // Activity (for callback binding)
                    .setCallbacks(viewModel.callbacks) // OnVerificationStateChangedCallbacks
                    .setForceResendingToken(resentToken).build()
            }
            if (options != null) {
                PhoneAuthProvider.verifyPhoneNumber(options)
            }
