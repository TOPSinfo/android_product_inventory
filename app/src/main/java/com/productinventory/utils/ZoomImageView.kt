...
class ZoomImageView : androidx.appcompat.widget.AppCompatImageView {
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        if (event?.action == MotionEvent.ACTION_UP && handlingDismiss) {
    ...
    override fun onDraw(canvas: Canvas) {
        super.onDraw(canvas)
        if (debugInfoVisible) {
            canvas.drawText(logText, Constants.FLOAT_10, height - Constants.FLOAT_10, textPaint)
            val drawableBound = displayRect?.let {
                "Drawable: $it"
            } ?: ""
            canvas.drawText(drawableBound, Constants.FLOAT_10, Constants.FLOAT_40, textPaint)
        }
    }
    ...
    private fun initTextPaint() {
        ...
        textPaint.textSize = Constants.FLOAT_40
    }
